...
export default class LikedPage {
    ...
    return `
      <section class="container view-transition-content">
        ...
        <div class="heading--container" aria-label="Heading container">
          <h1 aria-label="Liked Story">Liked Story</h1>
          <p aria-label="Deskripsi Liked Story" >Lihat semua Story yang kamu sukai!</p>
        </div>
  ...
  async afterRender() {
    ...
    try {
      ...
      console.error('Error initializing LikedPresenter:', error);
  ...
  showStories(stories) {
    ...
    if (!stories || stories.length === 0) {
      this.#storiesContainer.innerHTML =
        '<p>Belum ada Story yang anda Sukai.</p>';
      return;
    }
